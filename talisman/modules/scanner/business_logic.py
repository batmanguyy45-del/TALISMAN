...
async def _test_negative_values(
 ...
 try:
  baseline = await client.get(url, timeout=8)
 ...
 success_keywords = re.compile(r"success|accepted|created|order", re.IGNORECASE)
 noise_keywords = re.compile(r"invalid|error|expected|must be|should be", re.IGNORECASE)
  ...
  for value in ["-1", "-100", "-0.01", "0", "-9999"]:
   try:
    ...
    if r.status_code == baseline_status:
     continue
    text = r.text.lower()
    if not success_keywords.search(text):
     continue
    if noise_keywords.search(text):
     continue
    findings.append({
