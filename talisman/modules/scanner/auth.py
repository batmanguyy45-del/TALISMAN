...
LOGIN_SUCCESS_INDICATORS = [
 ...
 "redirecting", "my profile",
]
LOGIN_FAIL_INDICATORS = [
 "invalid", "incorrect", "failed", "error", "wrong",
 "not found", "doesn't match", "unauthorized", "invalid username",
 "invalid password", "access denied", "forbidden",
]
 ...
 **kwargs: Any,
 ...
 async with TalismanHTTPClient(proxy=proxy, timeout=15) as client:
  ...
  if login_info and not login_info.get("csrf_field"):
   has_js_csrf = False
   if "csrf" in (login_info.get("action") or "").lower():
    has_js_csrf = True
   if not has_js_csrf:
