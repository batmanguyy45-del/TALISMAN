...
PP_DETECTION_PAYLOADS = [
 ("__proto__[TLSMPP]=polluted", "URL query __proto__"),
 ("constructor.prototype.TLSMPP=polluted", "constructor.prototype"),
 ("__proto__.TLSMPP=polluted", "dot notation"),
]

PP_JSON_PAYLOADS = [
 '{"__proto__": {"TLSMPP": "polluted"}}',
 '{"constructor": {"prototype": {"TLSMPP": "polluted"}}}',
]
...
def _random_canary() -> str:
 return "TLSMPP" + "".join(random.choices(string.ascii_letters, k=10))
...
async def _test_query_pollution(url: str, client: TalismanHTTPClient) -> dict[str, Any] | None:
 ...
 for payload_str, desc in PP_DETECTION_PAYLOADS:
  ...
  test_url = test_url.replace("TLSMPP", canary)
  try:
...
async def _test_json_pollution(url: str, client: TalismanHTTPClient) -> dict[str, Any] | None:
 for payload in PP_JSON_PAYLOADS:
  canary = _random_canary()
  p = payload.replace("TLSMPP", canary)
  try:
   r = await client.post(
    url, content=p,
    ...
    if session:
     await session.add_finding(
      ...
      description=(
       ...
       "Unique canary injected into object prototype was reflected in response, "
